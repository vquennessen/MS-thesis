...
for (f in 1:length(folders)) {
  ...
  load(paste('~/Documents/MS-thesis/data/', folders[f], '/', 
             'BR_OR_2015_overfished/', num_sims, '_biomass.Rda', sep = ''))
  load(paste('~/Documents/MS-thesis/data/', folders[f], '/', 
             'BR_OR_2015_overfished/', num_sims, '_yield.Rda', sep = ''))
  load(paste('~/Documents/MS-thesis/data/', folders[f], '/', 
             'BR_OR_2015_overfished/', num_sims, '_effort.Rda', sep = ''))
...
final_plot <- ggdraw(fig2) + 
  draw_label(label = "MSY", x = 0.095, y = 0.705, size = 8.5)

# save plot
ggsave(final_plot,
       filename = paste('figS16_Black_Rockfish_overfished.png', sep = ''),
