FM_to_cause_depletion <- function(true_dep, A, R0, eq_time, species, eq_time) {
  
  # clear environment
  rm(list = ls())
  
  scales <- seq(0.001, 0.01, 0.001)
  ...
  for (s in scales) {
    ...
    S <- selectivity_at_age(L, fleets, alpha, beta, # selectivity at age
    ...
    E2[, 1:(eq_time - 1), ] <- rep(0.2, eq_time - 1)
    
    # Initialize FM and depletion levels
    fb_values <- seq(from = 0, to = 1, by = s)
    fn <- length(fb_values)
    ...
    for (fb in 1:fn) { 
      ...
      for (t in (rec_age + 1):(eq_time - 1)) {
        ...
        # biology
        PD <- pop_dynamics(1, t, 1, rec_age, max_age, n, SSB2, N2, W, Mat, A, 
                           R0, h, B0, Eps2, sigma_R, fb, E2, S, M, FM2, m, 
        ...
        # fishing
        catch2[, 1, t, 1] <- catch_at_age(1, t, 1, FM2, M, N2, A, fb, E2, 
                                          catch2, catch_form, season)
    ...
    title <- sprintf("Scale: %f", s)
    
    plot(fb_values, dep, ylim = c(-0.3, 1), xlim = c(0, 1), main = title)
