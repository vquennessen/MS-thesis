equilibrium_SAD <- function(a, cr, A, rec_age, max_age, n, W, R0,
  ...
  N2 <- FM2 <- array(rep(0, n*eq_timea), c(n, 1, eq_time, 1))
  ...
  if (stochasticity == T) {
    ...
    nuR2 <- array(rep(0, eq_time), c(1, eq_time, 1))
  }
  ...
  for (t in (rec_age + 1):(eq_time - 1)) {
    ...
    # biology
    PD <- pop_dynamics(a = 1, t, cr = 1, nm = 2, rec_age, max_age, n, SSB2, 
                       N2, W, Mat, A, R0, h, B0, Eps2, sigma_R, Fb, E2, S, M, 
