  ...
  if (v == 1) { 
    ...
    for (f in 1:length(folders)) {
      ...
      load(paste('~/Documents/MS-thesis/data/', folders[f], '/', 
                 species_list[1], '/', num_sims, '_biomass.Rda', sep = ''))
      load(paste('~/Documents/MS-thesis/data/', folders[f], '/', 
                 species_list[1], '/', num_sims, '_yield.Rda', sep = ''))
      load(paste('~/Documents/MS-thesis/data/', folders[f], '/', 
                 species_list[1], '/', num_sims, '_effort.Rda', sep = ''))
    ...
    if (v == 1) {
      ...
      source('200CR_extract_deterministic_values.R')
      deterministic <- read.csv('200CR_deterministic_values.csv')
      new_deterministic <- subset(deterministic, 
                                  FDR %in% FDRs & Species == Names[1])
    ...
    name_pt1 <- ifelse(v == 1, 
                       paste('200CR_dfinal_composite_', nruns, 'n', sep = ''), 
                       paste('200CR_transient_composite_', nruns, 'n', 
                             sep = ''))
    
    # save plot
    ggsave(final_plot,
           filename = paste(name_pt1, '.png', sep = ''),
           path = '~/Documents/MS-thesis/figures/200 year runs/',
