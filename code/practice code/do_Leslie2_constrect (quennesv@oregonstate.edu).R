...
#' M, t, n_init, f, Rfact) {
  ...
  # if f (fishing mortality) is not given, f = 0
  f = ifelse(exists("f"), f, 0)
  
  # if Rfact (annual increase in R) is not given, Rfact = 0
  Rfact = ifelse(exists("Rfact"), Rfact, 0)
  
  # Leslie matrix, unfished
  A0 = Leslie(max_age, a_mat, t_c, L_inf, k, t0, a2, b2, a, M, 0)
  
  # Leslie matrix, fished
  AF = Leslie(max_age, a_mat, t_c, L_inf, k, t0, a2, b2, a, M, f)
  ...
  nmm = matrix(rep(0, max_age), ncol = 1)
  ...
  D = matrix(rep(0, max_age + 1), ncol = 1)
